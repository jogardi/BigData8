...
def k_means_cost(X, clusters, label):
	...
	"*** YOUR CODE HERE ***"
	def distance_to_cluster(index_of_point):
		point = X[index_of_point, :]
		cluster = clusters[label[index_of_point]]
		return np.linalg.norm(point - cluster)
	
	cost = sum([distance_to_cluster(index_of_point) for index_of_point in range(m)])
